...
int main(int argc, char *argv[]) {
	...
	//printf("addrHoldingSwiHandlerAddr is %x\n",addrHoldingSwiHandlerAddr);
	...
	*addrHoldingSwiHandlerAddr = (unsigned) swiHandlerAddr; 
	...
	printf("in kernel.c \n");
	printf("kernel stack addr : %x, val at addr: %x\n",globArray, globArray[0]);
 	
	setUserConditions();
	printf ("finish seting user conditions: globArray addr: %x, globArray[0]:  %x \n", globArray, globArray[0]); 
...
void install_Handler(unsigned int swiHandlerAddr, unsigned int *uBootSwiAddr) {
	...
	offset = ((int)swiHandlerAddr - ((int) uBootSwiAddr + 0x8));
 	printf("difference btw ldr @ %x and addr holding addr %x is %x",swiHandlerAddr,
		uBootSwiAddr, offset);
	...
	printf(" corrected offset abs value of neg is %d", offset); 
